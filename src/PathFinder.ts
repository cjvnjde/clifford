...
export class PathFinder {
    ...
    init(initialPoint?: { x: number, y: number }) {
        if (initialPoint) {
    ...
    isWalkable(cellType: Elements) {
        return ![Elements.BRICK, Elements.STONE].includes(cellType)
    }

    moveToExplored(point: Point) {
    ...
    explorePoints(point: Point) {
        ...
        const isOnTheFloor = !this.isWalkable(underCellType) || [Elements.LADDER, Elements.HERO_LADDER, Elements.HERO_MASK_LADDER, Elements.OTHER_HERO_LADDER].includes(underCellType);
    ...
    findPoints(elements = [Elements.CLUE_KNIFE, Elements.CLUE_GLOVE, Elements.CLUE_RING]) {
        let foundPoints = [];
    ...
    findNearestPath(elements?: Elements[]) {
